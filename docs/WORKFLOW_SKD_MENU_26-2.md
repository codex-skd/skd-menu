...
|---|---|---|
| **snake_case** | `mod_id` en gradle.properties, assets/, packages Java | `skd_menu` |
| **PascalCase** | Clases Java principales | `SkdMenu` |
| **camelCase** | Variables, métodos, config keys | `skdMenu` |
| **Title Case** | Display name en README, CHANGELOG, docs, CurseForge | `SKD Menu` |

### Ficheros de documentación

| Fichero | Formato | Ejemplo |
|---|---|---|
| WORKFLOW | `WORKFLOW_<MOD_ID>_<MC-VERSION>.md` | `WORKFLOW_SKD_MENU_26-2.md` |
...
- Las clases Java principales deben seguir el naming del `mod_id` pero en **PascalCase**:
  - `skd_menu` → clase `SkdMenu`, no `Skd_menu` ni `SkdMenuMod`
...
├── docs/
│   ├── WORKFLOW_SKD_MENU_26-2.md       # Este documento
...
```bash
# Ejemplo: para minecraft/26.2/neoforge-26.2.0.32-beta/production
...
| Ejemplo | Significado |
|---|---|
| `skd_menu-26.2-neoforge-0.0.0-beta.3.jar` | NeoForge 26.2, beta 3 |
...
|---|---|---|
| Beta | `<mc-version>-neoforge-beta.X` | `26.2-neoforge-beta.1` |
| Release | `<mc-version>-neoforge-X.Y.Z` | `26.2-neoforge-1.0.0` |
...
```bash
# Beta
git tag -a 26.2-neoforge-beta.1 -m "v0.0.0-beta.1: First beta release"
git push origin 26.2-neoforge-beta.1

# Release estable
git tag -a 26.2-neoforge-1.0.0 -m "v1.0.0: First stable release"
git push origin 26.2-neoforge-1.0.0
```
...
git tag -a 26.2-neoforge-beta.3 -m "v0.0.0-beta.3: Bugfix release"
git push origin 26.2-neoforge-beta.3
